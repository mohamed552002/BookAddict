...
export class AuthApis extends BaseApi {
  ...
  public Login(email:string , password:string):Observable<any>{
    return this.http.post(this.apiUrl+"GetToken",{
      ...
      const user = new User(resData.id,resData.firstName,resData.lastName,resData.email,resData.roles,resData.token,resData.expireson,resData.isAuthenticated,resData.message);
      this.user.next(user);
